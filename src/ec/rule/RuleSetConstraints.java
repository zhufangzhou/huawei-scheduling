 ...
 * are to be mutated under the "default" priorityRule mutation operator.
 *
 ...
 * to specify that the system should make individuals with 0 rules 0.1 of the time, 1 priorityRule 0.2 of the time,
 * and 2 rules 0.7 of the time, you set <i>reset-num-sizes</i> to 3 (for priorityRule sizes up to but not including 3),
 * and then set  reset-size.0 to 0.1, reset-size.1 to 0.2, and reset-size.2 to 0.7.
 *
 * <p>Next the mutation parameters.  The default mutation procedure works as follows.  First, every priorityRule
 * in the ruleset is mutated.  It is up to the priorityRule to determine by how much or whether it will be mutated
 ...
 * item might or might not matter to you depending on whether or not your priorityRule interpreter differs depending on priorityRule order.
 *
 ...
 * @version 1.0 
 ...
 <td valign=top>(number of priorityRule set constraints)</td></tr>

 <tr><td valign=top><i>base.n</i>.<tt>name</tt><br>
 <font size=-1>String</font></td>
 <td valign=top>(name of priorityRule set constraint <i>n</i>)</td></tr>

 <tr><td valign=top><i>base.n</i>.<tt>reset-min-size</tt><br>
 <font size=-1>int >= 0 (default=0)</font></td>
 <td valign=top>(for priorityRule set constraint <i>n</i>, the minimum number of rules that rulesets may contain upon initialization (resetting), see discussion above)</td></tr>

 <tr><td valign=top><i>base.n</i>.<tt>reset-max-size</tt><br>
 <font size=-1>int >= <i>base.n</i>.<tt>reset-min-size</tt> (default=0)</font></td>
 <td valign=top>(for priorityRule set constraint <i>n</i>, the maximum number of rules that rulesets may contain upon initialization (resetting), see discussion above)</td></tr>

 <tr><td valign=top><i>base.n</i>.<tt>reset-num-sizes</tt><br>
 <font size=-1>int >= 0</font> (default=unset)</td>
 <td valign=top>(for priorityRule set constraint <i>n</i>, the number of sizes in the size distribution for initializtion, see discussion above)</td></tr>

 <tr><td valign=top><i>base.n</i>.<tt>reset-size</tt>.<i>i</i><br>
 <font size=-1>0.0 &lt; double &lt; 1.0</font></td>
 <td valign=top>(for priorityRule set constraint <i>n</i>, the probability that <i>i</i> will be chosen as the number of rules upon initialization, see discussion above)</td></tr>

 <tr><td valign=top><i>base.n</i>.<tt>p-add</tt><br>
 <font size=-1>0.0 &lt; double &lt; 1.0</font></td>
 <td valign=top>(the probability that a new priorityRule will be added, see discussion)</td></tr>

 <tr><td valign=top><i>base.n</i>.<tt>p-del</tt><br>
 <font size=-1>0.0 &lt; double &lt; 1.0</font></td>
 <td valign=top>(the probability that a priorityRule will be deleted, see discussion)</td></tr>
...
//    public final static String P_SIZE = "size";
    public final static String P_RULE = "priorityRule";  // our prototype
//    public static final int CHECK_BOUNDARY = 8;
    ...
    // probability of adding a random priorityRule to the priorityRule set
    public static final String P_ADD_PROB = "p-add";
    public double p_add;

    // probability of removing a random priorityRule from the priorityRule set
    public static final String P_DEL_PROB = "p-del";
    public double p_del;

    // probability of randomizing the priorityRule order in the priorityRule set
    ...
    /**
       The prototype of the PriorityRule that will be used in the RuleSet
    ...
    */
    ...
    /**
       ...
       to generate when calling reset() on this kind of PriorityRule.  The default
    ...
    */
    ...
    public void setup(final EvolutionState state, final Parameter base)
        ...
        // load my prototypical PriorityRule
